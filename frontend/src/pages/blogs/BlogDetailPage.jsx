...
import { insertReplyIntoTree, removeCommentFromTree } from '../../utils/commentTree';
...
export default function BlogDetailPage() {
  ...
  const handleLike = async () => {
    ...
    try {
      const { data } = await likeApi.toggle({ postId: post._id });
      setLiked(data.data.liked);
    } catch (err) {
      setLiked(previousLiked);
      setLikeCount(previousCount);
      showSnackbar(err.response?.data?.message || 'Like failed', 'error');
    }
  };
  ...
  if (postLoading) return <LoadingSection minHeight={360} />;
  ...
  if (!post) {
    return (
      <PageContainer>
        <EmptySection title="404" description="Post not found" />
      </PageContainer>
    );
  }
